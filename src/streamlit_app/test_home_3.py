...
import branca.colormap as cm # Import branca for colormaps (still useful for legend ranges)
...
def fetch_all_kpi_snapshots(
    ...
    if kpi_type == "vehicle_count":
        ...
        print(f"DEBUG: Unrecognized KPI type '{kpi_type}'. Using default legend ranges.")
    ...
    for i, ts in enumerate(available_timestamps_in_range):
        ...
        print("209: DEBUG: Geometry types:", current_map_data_gdf.geometry.geom_type.value_counts())
        # print("210: DEBUG: Geometry columns:", current_map_data_gdf.columns)
        print(current_map_data_gdf.head())
        print(current_map_data_gdf.head()[['geometry']])

        # We are no longer pre-calculating 'display_color' in Python.
        # The JS getColor function will handle this.
        ...
        print("224: DEBUG: Geometry types:", current_map_data_gdf.geometry.geom_type.value_counts())
        ...
        print("229: DEBUG: Geometry types:", current_map_data_gdf.geometry.geom_type.value_counts())
        
        if current_map_data_gdf.crs and current_map_data_gdf.crs.to_epsg() != 4326:
            print(f"Reprojecting from {current_map_data_gdf.crs} to EPSG:4326 for Leaflet compatibility.")
            current_map_data_gdf = current_map_data_gdf.to_crs(epsg=4326)
        
        if current_map_data_gdf.crs is None:
            print("CRS not found, setting CRS to EPSG:32633 (UTM zone for Berlin)")
            current_map_data_gdf.set_crs(epsg=32633, inplace=True)
        else:
            print(f"CRS is already set to {current_map_data_gdf.crs}.")
        ...
        for feature in geojson_dict['features']:
            ...
            # Removed 'display_color' from properties here, as JS will calculate
