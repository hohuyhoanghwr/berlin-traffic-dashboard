...
def fetch_all_kpi_snapshots(
    ...
    for ts in available_timestamps_in_range:
        ...
        # Fill NaN KPI values with 0 and ensure float type for coloring
        current_map_data_gdf['kpi_value'] = current_map_data_gdf['kpi_value'].fillna(0).astype(float)
        ...
        current_map_data_gdf['tooltip_content'] = current_map_data_gdf.apply(
            lambda row: f"<b>Road:</b> {row['name_road_segment']}<br><b>{kpi_type.replace('_', ' ').title()}:</b> {row['kpi_value']:.2f}",
            axis=1
        )
        ...
        for feature in geojson_dict['features']:
            ...
            original_row = current_map_data_gdf[current_map_data_gdf['segment_id'] == segment_id].iloc[0]
...
if st.session_state["load_triggered"]:
    ...
    # auto_playing is already set to False when "Load Map" is clicked, or kept from previous state
    ...
    def create_map_html(
        ...
        # Define color scales based on KPI type
        color_scale_js = ""
        if selected_kpi_type == "vehicle_count":
            color_scale_js = """
            function getColor(value) {
                // console.log("Vehicle Count KPI value:", value); // Debugging
                return value > 1000 ? '#662506' :
                       value > 500  ? '#993404' :
                       value > 200  ? '#CC4C02' :
                       value > 100  ? '#EC7014' :
                       value > 50   ? '#FE9929' :
                       value > 20   ? '#FEC44F' :
                       value > 10   ? '#FED976' :
                                      '#FFFFCC'; // Lightest color for 0-10
            }
            """
        elif selected_kpi_type == "average_speed":
            color_scale_js = """
            function getColor(value) {
                // console.log("Average Speed KPI value:", value); // Debugging
                return value > 60 ? '#006837' : // Dark Green (fast)
                       value > 50 ? '#31a354' :
                       value > 40 ? '#78c679' :
                       value > 30 ? '#addd8e' :
                       value > 20 ? '#f7fcb9' : // Light Green/Yellow
                       value > 10 ? '#fee08b' : // Light Orange
                       value > 0  ? '#fc8d59' : // Orange
                                    '#d73027'; // Red (slow/stopped)
            }
            """
        else:
            color_scale_js = """
            function getColor(value) {
                return '#808080'; // Default grey if KPI type is unknown
            }
            """

        # Define legend ranges based on KPI type
        legend_ranges_js = ""
        if selected_kpi_type == "vehicle_count":
            legend_ranges_js = """
            const legendRanges = [
            ...
            legend_ranges_js = """
            const legendRanges = [
                { value: '60+ km/h', color: '#006837' },
                { value: '51-60 km/h', color: '#31a354' },
                { value: '41-50 km/h', color: '#78c679' },
                { value: '31-40 km/h', color: '#addd8e' },
                { value: '21-30 km/h', color: '#f7fcb9' },
                { value: '11-20 km/h', color: '#fee08b' },
                { value: '1-10 km/h', color: '#fc8d59' },
                { value: '0 km/h', color: '#d73027' }
            ];
            """
        
        # Determine KPI display name for legend
        kpi_display_name = selected_kpi_type.replace('_', ' ').title()
        ...
        <body>
            ...
            <script>
                ...
                function styleFeature(feature) {{
                    ...
                    // console.log("Styling feature with KPI value:", value); // Debugging KPI values
                    return {{
                ...
                function startAnimation() {{
                    ...
                    animationInterval = setInterval(() => {{
                        ...
                        if (currentAnimationIndex <= animationEndIndex) {{
                            ...
                            currentAnimationIndex = animationStartIndex; // Loop back to start
                            updateMapLayer(); // Update for the first frame of the new loop
                            // For stopping at end: stopAnimation();
                        }}
                    }}, animationSpeed);
                }}
                ...
                function addLegend(mapInstance) {{
                    ...
                    legend.onAdd = function (map) {{
                        ...
                        // Corrected typo: kpiDisPlayName -> kpiDisplayName
                ...
                (function() {{
                    // Check if map is already initialized (e.g., if Streamlit re-rendered the component
                    // but the iframe itself wasn't fully reloaded, or for initial load)
                    if (map) {{
                        // If map exists, just update its data and animation state
                        // This handles Streamlit reruns where the iframe content is re-injected
                        // but the Leaflet map object might still be in memory.
                        // However, st.components.v1.html usually reloads the iframe entirely.
                        // The primary purpose of this check is for robustness.
                        
                        // Stop any existing animation before re-setting
                        stopAnimation(); 
                        currentAnimationIndex = initialCurrentIndexFromPython;
                        updateMapLayer(); // Update with new data and initial index
                        if (autoPlayOnLoad) {{
                            startAnimation();
                        }}
                        return; // Exit if map was already initialized
                    }}
