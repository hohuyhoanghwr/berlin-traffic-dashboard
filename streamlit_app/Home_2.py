...
@st.cache_data(show_spinner="Loading GeoJSON snapshots from MongoDB for animation...")
...
def create_map_html(
    ...
    if "speed" in selected_kpi_type_label.lower(): # Check if it's a speed KPI
        color_scale_js = """
        function getColor(d) {
            return d > 80 ? '#00441b' : // Dark Green (fast)
                   d > 60 ? '#238b45' :
                   d > 40 ? '#41ab5d' :
                   d > 20 ? '#74c476' :
                   d > 0  ? '#a1d99b' : // Light Green (slow)
                            '#e5f5e0'; // Very Light Green (no speed/zero)
        }
        """
        # Dynamic legend ranges for speed
        legend_ranges_js = """
        const legendRanges = [
            { value: '80+', color: '#00441b' },
            { value: '61-80', color: '#238b45' },
            { value: '41-60', color: '#41ab5d' },
            { value: '21-40', color: '#74c476' },
            { value: '1-20', color: '#a1d99b' },
            { value: '0', color: '#e5f5e0' }
        ];
        """
    ...
    <head>
        ...
        <style>
            ...
            #map {{ height: 80vh; width: 100%; border-radius: 8px; }} /* Adjusted height here */
            .map-controls {{
                position: absolute;
                bottom: 10px;
                left: 50%;
...
components.html(map_html, height=850, scrolling=False) # Re-added key
...
with col1_sidebar:
    if st.button("Start Animation", key="start_animation_btn", disabled=st.session_state["auto_playing"]):
        ...
        # No st.rerun() here. Let the JS handle the animation loop.
        # The next rerun will happen when a control is changed or animation is stopped.
        # The map will re-render once due to auto_play_on_load_flag being true, triggering JS startAnimation.
        st.experimental_rerun() # Use experimental_rerun to force a rerun without blocking

with col2_sidebar:
    if st.button("Stop Animation", key="stop_animation_btn", disabled=not st.session_state["auto_playing"]):
        st.session_state["auto_playing"] = False
        # No st.rerun() here. The JS will stop its interval.
        # The map will re-render on the next control change or if start is pressed.
        st.experimental_rerun() # Use experimental_rerun to force a rerun without blocking


# --- Time Slider for Display (No continuous reruns) ---
# This slider will *not* cause continuous reruns.
...
# If auto_playing is true, we need a mechanism to keep the app "active"
# without continuously re-rendering the HTML component.
# This is where the challenge lies without bi-directional communication.
# For now, we rely on the JS to update the time display.
# Streamlit will only rerun if a control is changed or a button is pressed.
# The previous block that caused continuous reruns is removed.
# The user's request is that the slider should only be for display, not trigger anything.
# But if it's for display, it needs to be updated by JS.
# Since we cannot update Streamlit's slider from JS without st_javascript,
# the slider will only show the *initial* frame or the frame it was on when
# a control was last changed/animation stopped.

# To address the "time slider during animation" for display,
# we would typically need bi-directional communication (e.g., st_javascript).
# Without it, the slider in Streamlit will NOT smoothly follow the animation.
# It will only update when a Streamlit rerun happens (e.g., user stops animation, changes filter).
# The `timeDisplay` inside the HTML component is the true real-time indicator.

# Let's add a small sleep here to prevent Streamlit from consuming too much CPU
# if it's constantly checking for state changes, even without reruns.
# This is a minor optimization, not directly related to the flashing fix.
# It ensures Streamlit doesn't spin too fast in idle state.
time.sleep(0.1) # Small sleep to reduce CPU usage when idle
